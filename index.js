...
// Keep server alive - prevent crashes from killing the web panel
process.on('uncaughtException', (error) => {
  console.error('[SERVER] Uncaught Exception (server kept alive):', error.message);
});
...
// Suppress all console output (quiet mode) - DISABLED for terminal logs
...
app.post('/api/start', async (req, res) => {
  try {
    ...
    try {
      const accountIssuePath = path.join(__dirname, 'Data/system/database/botdata/account_issue.txt');
...
app.post('/api/restart', async (req, res) => {
  try {
    if (botModule && botModule.isBotRunning && botModule.isBotRunning()) {
      console.log('[🔄 Bot Restart] Stopping existing bot...');
      if (botModule.stopBot) {
        await botModule.stopBot();
      }
    }
    ...
    Object.keys(require.cache).forEach(key => {
      if (key.includes(path.join(__dirname, 'CHAND')) || key.includes('fca-unofficial') || key.includes('cookieManager')) {
        delete require.cache[key];
      }
    });
    ...
    res.json({ success: true, message: 'Bot restarted in current process' });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
});
    ...
    if (botStarted) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(' |____||____/|_| \\_\\____/ \\____/ |_|  ');
          console.log('');
          console.log('+---------------------------------------------------+');
          console.log('|           [X] BOT STARTUP FAILED                 |');
          console.log('+---------------------------------------------------+');
          console.log('|  Error: ' + error.message.substring(0,43).padEnd(43) + '|');
          console.log('|                                                   |');
          console.log('|  Possible Solutions:                              |');
          console.log('|  1. Upload new appstate from Facebook            |');
          console.log('|  2. Add valid cookies.txt in root                |');
          console.log('|  3. Restart bot after updating credentials       |');
          console.log('+---------------------------------------------------+');
          console.log('');
        }
      }, 3000);
